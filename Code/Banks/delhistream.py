...
from delhibanks import get_branch_details_by_zone,get_address_of_branch,get_branch_name_by_address,get_ifsc_code_by_address,get_ifsc_code_of_branch,get_address_by_ifsc_code,get_branch_name_and_ifsc_by_partial_ifsc,get_bank_details_by_branch,get_bank_details_by_ifsc,get_bank_details_by_address
...
def run_delhi_app():
    ...
    # Add functionality for other options
        ...
        st.title("Banks information")
        input_zone = st.text_input("Enter the zone to get Bank information:")
        if st.button("Get Banks in Zone"):
            banks_in_zone = get_branch_details_by_zone(input_zone)
            st.write(f"Banks in {input_zone}: ")
            st.write(banks_in_zone)
        
          

        branch_name = st.text_input("Enter the Branch name to get Bank information:")
        if st.button("Get Bank information of Branch"):
            bank_information = get_bank_details_by_branch(branch_name)
            if(bank_information):
                st.write(f"Banks information of {branch_name} Branch: ")
                st.write(bank_information)
        else:
            st.write("No Information found")

        ifsc_code = st.text_input("Enter the IFSC Code to get Bank information:")
        if st.button("Get Bank information of IFSC Code"):
            bank_information = get_bank_details_by_ifsc(ifsc_code)
           
                #for ifsc in bank_information:
                    #st.write(f"Information of banks with {ifsc_code} IFSC Code: ")
            st.write(bank_information)
        
    
        input_address = st.text_input("Enter the Location to get Bank information:")
        if st.button("Get Bank information of Location"):
            bank_information = get_bank_details_by_address(input_address)
            if(bank_information):
                st.write(f"Information of Banks at {input_address}: ")
                st.write(bank_information)
            else:
                st.write("No Information found")
    
    
        branch_name = st.text_input("Enter the Branch name to get Bank Location:")
        if st.button("Get Bank Location of Branch"):
            bank_location = get_address_of_branch(branch_name)
            if (bank_location):
                for location in bank_location:
                    st.write(f"Location of {branch_name} Branch: ",location)
            else:
                st.write("No location found")
    

        branch_name = st.text_input("Enter the Branch name to get IFSC code:")
        if st.button("Get IFSC Code of Branch"):
            bank_ifsc = get_ifsc_code_of_branch(branch_name)
            if(bank_ifsc):
                for ifsc in bank_ifsc:
                    st.write(f"IFSC Code of {branch_name} Branch: ",ifsc)
            else:
                st.write("No IFSC Code Found ")
            
                 
        
        input_address = st.text_input("Enter the Location of Bank to get IFSC code:")
        if st.button("Get IFSC Code by Location"):
            bank_ifsc = get_ifsc_code_by_address(input_address)
            if(bank_ifsc):
                for ifsc in bank_ifsc:
                    st.write(f"IFSC Code of Banks at {input_address}: ", ifsc)
            else:
                st.write("No IFSC Code Found")
    
        input_address = st.text_input("Enter the Location of Bank to get Branch Names:")
        if st.button("Get Branch Name by Location"):
            bank_branch = get_branch_name_by_address(input_address)
            if(bank_branch):
                for branch in bank_branch:
                    st.write(f"Branch Name of Banks at {input_address}: ", branch)
            else:
                st.write("No Branch Found")

        input_ifsc = st.text_input("Enter the IFSC code of Bank to get Branch Names:")
        if st.button("Get Branch Names by IFSC"):
            bank_branches, bank_ifsc_codes = get_branch_name_and_ifsc_by_partial_ifsc(input_ifsc)
            if bank_branches and bank_ifsc_codes: 
                st.write(f"Branch Name: {bank_branches}, IFSC Code: {bank_ifsc_codes}")
            else:
                st.write("No branches found for the given IFSC code.")

    
        input_ifsc = st.text_input("Enter the IFSC code of Bank to get Bank Location:")
        if st.button("Get Location by IFSC"):
            bank_location, bank_ifsc_codes = get_address_by_ifsc_code(input_ifsc)
            if bank_location and bank_ifsc_codes: 
                st.write(f"IFSC Code: {bank_ifsc_codes}, Location: {bank_location}")
            else:
                st.write("No Locations found for the given IFSC code.")
