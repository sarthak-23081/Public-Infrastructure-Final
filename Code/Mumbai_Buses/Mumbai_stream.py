...
def run_public_transport():
    ...
    if selected_option == "Metro":
        ...
        depot_route = st.text_input("Enter the depot name to get the bus route information:")
        if st.button("Bus Route Information"):
            answer = get_buses_in_depot(depot_route)
            st.write("Answer:", answer)

        depot_name = st.text_input("Enter the depot to get the count of bus stations:")
        if st.button("Get Bus Station Count"):
            answer2 = count_bus_stations_in_depot(depot_name)
            st.write("Answer:", answer2)
            
        depot_name2 = st.text_input("Enter the depot to get the list of bus stations:")
        if st.button("Get Bus Station List"):
            answer3 = list_bus_stations_in_depot(depot_name2)
            st.write("Answer:", answer3)
            
        depot_name3 = st.text_input("Enter the depot to get the major operations:")
        if st.button("Get major operations Information"):
            answer4 = get_major_operations_of_depot(depot_name3)
            st.write("Answer:", answer4)
            
        depot_name4 = st.text_input("Enter the code to get the depot:")
        if st.button("Get depot Information"):
            answer5 = get_depot_by_code(depot_name4)
            st.write("Answer:", answer5)
            
        depot_name5 = st.text_input("Enter the route to get the bus station information:")
        if st.button("Get route-bus station Information"):
            answer6 = get_bus_stations_by_route(depot_name5)
            st.write("Answer:", answer6)
            
        depot_name6 = st.text_input("Enter the major operation to get bus information:")
        if st.button("Get bus Information"):
            answer7 = get_buses_by_major_operation(depot_name6)
            st.write("Answer:", answer7)

        starts_with_letter = st.text_input("Enter a letter to find buses starting with that letter (e.g., A):")
        if starts_with_letter:
            starts_with_answer = get_buses_starting_with(starts_with_letter)
            st.write(f"Buses Starting with {starts_with_letter}:", starts_with_answer)
            
        depot_name7 = st.text_input("Enter the region to get count of bus depots:")
        if st.button("Get bus depot Information"):
            answer8 = count_buses_in_region(depot_name7)
